...
import { createAppViewTelemetryLoggedAction } from '../../shared/actions/actions';
...
// export function logInitialTelemetryEvents(store: Redux.Store) {
export function logInitialTelemetryEvents(store: any) {
    ...
    let spzaUserIDPayload = {
        eventName: Constants.Telemetry.Action.SpzaUserId,
        actionModifier: userIdInfo.newUserModifier,
        data: userIdInfo.spzaUserId
    };

    instrument.probe<any>('logOneTimeInfo', spzaUserIDPayload);

    // Logs user tenant info if logged in
    logTenantInfo(state.users, false, userIdInfo.spzaUserId);

    let embedHostName = getEmbedHostName(state.config.embedHost);
    ...
    let landingPagePayload: ITelemetryData = {
        page: getWindow().location.href,
        action: Constants.Telemetry.Action.PageLoad,
        actionModifier: Constants.Telemetry.ActionModifier.End,
        details: JSON.stringify(landingDetails)
    };
    instrument.probe<ITelemetryData>('logTelemetryInfo', landingPagePayload);
    ...
    if (!embedHostUtils.hasDynamicData(state.config.embedHost)) {
        ...
        let loadEventEnd = (timing.loadEventEnd !== 0 ? timing.loadEventEnd : timeAtTelemetryMethodCall);
        // The time taken for page load once the page is received from the server
        ...
        let navigationToPageLoad = loadEventEnd - timing.navigationStart;
